...
class Cop:
    def __init__(self, board):
        flag1 = 0
        while (flag1 == 0):
            self.positionX = random.randint(1, c_t_m.n-2)
            self.positionY = random.randint(1, c_t_m.n-2)
            if (board[int(self.positionX)][int(self.positionY)]==0):
                flag1 = 1
    
    def move(self, move_plan, worlds_list_copy):
        t = len(worlds_list_copy) - 1
        board = worlds_list_copy[t]
...
class Planner:
    ...
    def thread_function(self, worlds_list_copy):
        self.worlds_list_copy = worlds_list_copy
