...
class World:
    ...
    def move(self):
        ...
        if ((len(self.worlds_list_copy)-1) % 5 == 0):
            self.plan.thread_function(self.worlds_list_copy)
            self.show_moves()
        self.cops.move(self.plan.cops_planner.move_plan, self.worlds_list_copy)
        self.thief.move(self.plan.thief_planner.move_plan, self.worlds_list_copy)
        self.replacement()
    ...
    def replacement(self):
        ...
        #self.worlds_list_copy = self.worlds_list
        #print(self.worlds_list[len(self.worlds_list)-1])
        for i in range(number_of_cops):
                ...
                for i in range(gate_width):
                    if (self.positionX[i] == 0):
                        if (self.positionY[i]+1<n):
                            self.positionX[i] = self.positionX[i]
                            self.positionY[i] = self.positionY[i]+1
                        else:
                            self.positionX[i] = self.positionX[i]+1
...
class Walls:
    def __init__(self):
        ...
        self.positionX.append(random.randint(1, n-2))
        self.positionY.append(random.randint(1, n-2))
        flag1 = False
        while (flag1 == False):
